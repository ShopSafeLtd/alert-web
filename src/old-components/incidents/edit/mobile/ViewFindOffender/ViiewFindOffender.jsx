...
const ViewFindOffender = ({
  ...
  // effects
  useEffect(() => {
    setBackLinkTo(`${basePath}/offenders/find`);
  ...
  const handleNext = offender => {
    if (!loading) {
      addExistingOffender(offender);
      history.push(`${basePath}/offenders`);
    }
  };
  ...
  return loading ? (
    ...
    <Page>
      {data.offender.images !== undefined && data.offender.images.length > 0 ? (
        ...
        <OffenderAvatar id={data.offender} />
      )}
      ...
      <FullWidthButton
        ...
        onClick={() => handleNext(data.offender.id)}
      />
    </Page>
  );
};

export default ViewFindOffender;
