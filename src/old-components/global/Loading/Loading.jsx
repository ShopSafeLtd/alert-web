...
const Page = styled.div`
  width: 100%;
  ${({ noHeight }) => !noHeight && 'height: 100%;'} flex: 1;
  background: #fff;
  display: flex;
...
class Loading extends Component {
  render() {
    const { text, noHeight } = this.props;
    const logo = window.localStorage.getItem('currentSchemeLogo');
    return (
      <Page noHeight={noHeight}>
        <Grow />
        <ImgContainer>
          <Img
        ...
        {text !== undefined && (
          <LoadingText variant="subtitle2">{text}</LoadingText>
        )}
        <Grow />
        {!!logo && (
          <Partnership variant="caption">In partnership with</Partnership>
        )}
        {!!logo && (
          <Image src={window.localStorage.getItem('currentSchemeLogo')} />
        )}
      </Page>
    );
  }
