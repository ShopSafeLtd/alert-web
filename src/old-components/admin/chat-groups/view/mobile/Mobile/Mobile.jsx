...
class Mobile extends PureComponent {
  ...
  render() {
    const { chatId, loading, chat } = this.props;
    ...
    const fabs = [
      {
        index: 0,
        icon: <EditIcon />,
        to: `/admin/chat-groups/edit/${chatId}`
      },
      {
        index: 1,
        icon: <EditIcon />,
        to: `/admin/chat-groups/users/${chatId}`
      }
    ];
    ...
    return (
      <Page>
        ...
        >
          <Details chat={chat} loading={loading} />
          <Users chat={chat} loading={loading} />
        </SwipeableViews>
