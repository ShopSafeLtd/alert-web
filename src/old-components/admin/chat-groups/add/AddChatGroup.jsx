...
const AddChatGroup = ({ location, history }) => {
  ...
  const steps = [
    { step: 0, label: 'Chat Name', url: '/admin/chat-groups/add' },
    { step: 0, label: 'Users', url: '/admin/chat-groups/add/users' }
  ];
  ...
  useEffect(() => {
    ...
    setBottomNav(false);
    location.pathname !== '/admin/chat-groups/add' &&
      history.push('/admin/chat-groups/add');
    return () => {
      setTitle('');
      setBottomNav(true);
    };
  ...
  // mutations
  const [addChat] = useMutation(mutation, {
  ...
  const handleNext = () => {
    if (step === 0) {
      ...
      history.push('/admin/chat-groups');
    }
  };
          ...
          <Page>
            <FormContainer elevation={1}>
              ...
              <Content>
                ...
                />
                <Route
                  path="/admin/chat-groups/add/users"
                  render={() => (
                    <Users
                      users={data.users}
