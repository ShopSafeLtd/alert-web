...
const OffenderWarnings = ({ history }) => {
  ...
  // queries
  const { data, loading } = useQuery(AllOffenderLabels, {
    variables: {
    ...
    <MediaQuery minDeviceWidth={1024}>
      {matches => (
        <Page>
            ...
            {!!loading && loading ? (
              ...
              <Empty>
                ...
                <Button
                  ...
                  color="primary"
                  component={Link}
                  to="/admin/offender-warnings/add"
                >
              ...
              <List>
                {data.tags.map(({ id, name }) => (
                  <ListItem
                    key={id}
                    onClick={() =>
                      history.push(`/admin/offender-warnings/edit/${id}`)
                    }
                  >
          ...
          {!!data &&
            data.tags.length > 0 && (
              <FAB bottom to="/admin/offender-warnings/add" />
            )}
        </Page>
      )}
    </MediaQuery>
