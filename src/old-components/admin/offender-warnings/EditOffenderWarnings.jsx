...
const EditOffenderWarnings = ({ setActions, match, history }) => {
  ...
  useEffect(() => {
    ...
    setBackLinkTo(`/admin/offender-warnings`);
    setActions([
  ...
  // mutations
  const [updateWarning] = useMutation(Update);
  const [deleteOffenderWarning] = useMutation(DeleteMutation, {
    update: (store, { data: { deleteTag } }) => {
      ...
      data.tags = data.tags.filter(({ id }) => id !== deleteTag.id);
      store.writeQuery({
        query: AllOffenderLabels,
        data,
        variables: {
  ...
  const handleSave = () => {
    validate()
      .then(async () => {
        ...
        history.push('/admin/offender-warnings');
      })
      .catch(() => {});
  };
