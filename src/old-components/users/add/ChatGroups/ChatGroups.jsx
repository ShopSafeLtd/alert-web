...
class ChatGroups extends PureComponent {
  ...
  render() {
    ...
    return (
      <Page>
        <Header>
          <HeaderText>Chat Groups</HeaderText>
          <HeaderSubText>
            Select the chat groups that you want this user to be a member of,
            this will allow the user to chat with the other members of this
            group.
          </HeaderSubText>
        </Header>
        {!loading ? (
          chatGroups !== undefined && (
            <List>
              {chatGroups.map(({ name, id }) => (
                <ListItem key={id} onClick={() => toggleChatGroup(id)}>
