...
export const normalizeMobileNumber = (
  value: null | string | undefined
): null | string | undefined => {
  if (!value) return value;
  const digits = value.replaceAll(/\D/g, '');
  const plus = value.startsWith('+') ? '+' : '';
  // Detect country code by finding the longest matching prefix in the digits.
  // If the digit immediately after the country code is '0', strip it.
  // This corrects legacy numbers stored with a domestic leading 0 retained
  // after the country code (e.g. +4407911123456 → +447911123456).
  const countryCodes = [
    '1',
    '7',
    '20',
  ...
  // Sort longest-first so more specific codes match before shorter prefixes.
  const sorted = [...countryCodes].sort((a, b) => b.length - a.length);
  for (const cc of sorted) {
    if (digits.startsWith(cc) && digits[cc.length] === '0') {
      return plus + cc + digits.slice(cc.length + 1);
    }
  }
  return value;
};
