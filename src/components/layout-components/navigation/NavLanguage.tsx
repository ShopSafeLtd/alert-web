...
const SelectedLanguage = ({ locale }: { locale: string }) => {
...
export const NavLanguage = ({ configDisplay }: Props) => {
	const locale = useStoreState(state => state.theme.locale)
	const onLocaleChange = useStoreActions(actions => actions.theme.changeLocale)
