...
const OffenderTable = ({
  ...
  const tableMetaData = useMemo(
    () => metadata.find((item) => item.key === 'offendersTable'),
    [metadata]
  );
  ...
  const onSubmitColumns = (formData: { [i: string]: string }) => {
    setMetadata([
      ...metadata.filter((item) => item.key !== 'offendersTable'),
      {
        key: 'offendersTable',
        type: 'table',
        ...tableMetaData,
        columns: OffenderColumns.filter(
    ...
    <>
      <div>
        ...
        <Table
          ...
          columns={OffenderColumns.filter((column) =>
          ...
          pagination={{
            ...
            total: data?.offenderTableReport?.total || 0,
          }}
          size="small"
        />
      </div>
          ...
          <Form
            initialValues={
              tableMetaData?.columns
                ...
                : Object.fromEntries(
                    OffenderColumns.map((b) => [b.key as string, true])
                  )
            }
            onFinish={onSubmitColumns}
          >
            {OffenderColumns.map((column) => (
              <Form.Item
                key={column.key}
                label={column.title}
                name={column.key}
