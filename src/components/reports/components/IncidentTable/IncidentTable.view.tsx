import type { IncidentTableReportQuery } from '#/views/reports/incident-table/__generated__/incident-table-report-query.generated';
import type { MetaData } from '#/views/reports/types';

import {
  IncidentColumns,
  type IncidentTableData,
...
const IncidentTable = ({
  ...
  const tableMetaData = useMemo(
    () => metadata.find((item) => item.key === 'incidentsTable'),
    [metadata]
  );
  ...
  const onSubmitColumns = (formData: { [i: string]: string }) => {
    setMetadata([
      ...metadata.filter((item) => item.key !== 'incidentsTable'),
      {
        key: 'incidentsTable',
        type: 'table',
        ...tableMetaData,
        columns: IncidentColumns.filter(
    ...
    <>
      <div>
        ...
        <Table
          ...
          columns={IncidentColumns.filter((column) =>
          ...
          pagination={{
            ...
            total: data?.incidentTableReport?.total || 0,
          }}
          size="small"
        />
      </div>
          ...
          <Form
            initialValues={
              tableMetaData?.columns
                ...
                : Object.fromEntries(
                    IncidentColumns.map((b) => [b.key as string, true])
                  )
            }
