...
type Incident = NonNullable<
  BusinessLossPreventionDashboardQuery['businessLossPreventionDashboard']['recentIncidents']
>[number];

interface LpRecentIncidentsProps {
  incidents: Incident[] | null | undefined;
  loading: boolean;
}

const LpRecentIncidents: React.FC<LpRecentIncidentsProps> = ({
  incidents,
  loading,
}) => {
  const intl = useIntl();
  ...
  return (
    <Card
      extra={
        <Typography.Text style={{ fontSize: 12 }} type="secondary">
          {intl.formatMessage({
            defaultMessage: 'See full list below ↓',
          })}
        </Typography.Text>
      }
      style={{ height: '100%' }}
    ...
    >
      {loading && !incidents ? (
        ...
        <Table
          columns={columns}
          dataSource={incidents.slice(0, 10)}
          onRow={(record) => ({
...
export default LpRecentIncidents;
