...
import { useIcelandImportDataMutation } from 'graphql/imports/__generated__/iceland-import.generated';
import { useAtomValue } from 'jotai/index';
import { useState } from 'react';

interface FormData {
  groups: string[];
  icelandData: UploadFile[];
}
...
const useIcelandImport = (): Return => {
  const schemeId = useAtomValue(currentSchemeIdAtom);
  const [saving, setSaving] = useState(false);
  const [form] = Form.useForm<FormData>();
  const [importData] = useIcelandImportDataMutation({
    onCompleted: () => {
...
export default useIcelandImport;
