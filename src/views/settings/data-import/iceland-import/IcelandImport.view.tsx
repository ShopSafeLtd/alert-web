...
import useIcelandImport from './useIcelandImport';

const IcelandImport = () => {
  const intl = useIntl();
  const { documentUploadProps, form, onSubmit, saving } = useIcelandImport();

  return (
    <div style={{ padding: 20 }}>
      <Card>
        <Typography.Title level={4}>
          <FormattedMessage defaultMessage="Iceland Import" />
        </Typography.Title>
        <Typography.Text>
          <FormattedMessage defaultMessage="Select a CSV or XLSX file from Iceland to import it." />
        </Typography.Text>
        <Form form={form} layout="vertical" onFinish={onSubmit}>
          <Form.Item
            ...
            tooltip={intl.formatMessage({
              defaultMessage: 'Upload the CSV or XLSX file from Iceland.',
            })}
          >
            <Upload
