...
import useNextImport from './useNextImport';

const NextImport = () => {
  const intl = useIntl();
  const { documentUploadProps, form, onSubmit, saving } = useNextImport();

  return (
    <div style={{ padding: 20 }}>
      <Card>
        <Typography.Title level={4}>
          <FormattedMessage defaultMessage="Next Import" />
        </Typography.Title>
        <Typography.Text>
          <FormattedMessage defaultMessage="Select a CSV or XLSX file from Next to import it." />
        </Typography.Text>
        <Form form={form} layout="vertical" onFinish={onSubmit}>
          <Form.Item
            ...
            tooltip={intl.formatMessage({
              defaultMessage: 'Upload the CSV or XLSX file from Next.',
            })}
          >
            <Upload
