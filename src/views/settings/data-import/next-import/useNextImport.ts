...
import { useNextImportDataMutation } from 'graphql/imports/__generated__/next-import.generated';
...
const useNextImport = (): Return => {
  const schemeId = useAtomValue(currentSchemeIdAtom);
  const [saving, setSaving] = useState(false);
  const [form] = Form.useForm<FormData>();
  const [importData] = useNextImportDataMutation({
    onCompleted: () => {
