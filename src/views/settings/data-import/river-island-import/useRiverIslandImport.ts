...
import { useRiverIslandImportDataMutation } from 'graphql/imports/__generated__/river-island-import.generated';
import { useAtomValue } from 'jotai/index';
import { useState } from 'react';

interface FormData {
  groups: string[];
  riverIslandData: UploadFile[];
}
...
const useRiverIslandImport = (): Return => {
  const schemeId = useAtomValue(currentSchemeIdAtom);
  const [saving, setSaving] = useState(false);
  const [form] = Form.useForm<FormData>();
  const [importData] = useRiverIslandImportDataMutation({
    onCompleted: () => {
...
export default useRiverIslandImport;
