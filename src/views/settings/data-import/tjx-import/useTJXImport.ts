...
import { useTjxImportDataMutation } from 'graphql/imports/__generated__/tjx-import.generated';
...
const useTJXImport = (): Return => {
  const schemeId = useAtomValue(currentSchemeIdAtom);
  const [saving, setSaving] = useState(false);
  const [form] = Form.useForm<FormData>();
  const [importData] = useTjxImportDataMutation({
    onCompleted: () => {
  ...
  const onSubmit = async ({ group }: FormData) => {
    try {
      setSaving(true);
