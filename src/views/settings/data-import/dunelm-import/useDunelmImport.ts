...
import { useDunelmImportDataMutation } from 'graphql/imports/__generated__/dunelm-import.generated';
import { useAtomValue } from 'jotai/index';
import { useState } from 'react';

interface FormData {
  dunelmData: UploadFile[];
  groups: string[];
}
...
const useDunelmImport = (): Return => {
  const schemeId = useAtomValue(currentSchemeIdAtom);
  const [saving, setSaving] = useState(false);
  const [form] = Form.useForm<FormData>();
  const [importData] = useDunelmImportDataMutation({
    onCompleted: () => {
...
export default useDunelmImport;
