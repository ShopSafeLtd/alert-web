import type { OffenderEngagementQuery } from '#/graphql/engagement/queries/__generated__/offender-engagement.generated';

import { useOffenderEngagementQuery } from '#/graphql/engagement/queries/__generated__/offender-engagement.generated';
import { useMemo, useState } from 'react';

type UserEngagement = NonNullable<
  OffenderEngagementQuery['offenderEngagement']['users'][0]
>;

interface UseOffenderEngagementDetailProps {
  offenderId: string;
}

interface UseOffenderEngagementDetailReturn {
  engagementData: OffenderEngagementQuery['offenderEngagement'] | null;
...
const useOffenderEngagementDetail = ({
  offenderId,
}: UseOffenderEngagementDetailProps): UseOffenderEngagementDetailReturn => {
  ...
  const { data, loading } = useOffenderEngagementQuery({
    variables: {
      offenderId,
    },
  });
  ...
  const filteredAndSortedUsers = useMemo(() => {
    if (!data?.offenderEngagement.users) return [];

    let filtered = data.offenderEngagement.users;
  ...
  return {
    engagementData: data?.offenderEngagement || null,
    filterStatus,
...
export default useOffenderEngagementDetail;
