...
import { useDocumentEngagementQuery } from '#/graphql/engagement/queries/__generated__/document-engagement.generated';
...
interface UseDocumentEngagementDetailProps {
  documentId: string;
}

interface UseDocumentEngagementDetailReturn {
...
const useDocumentEngagementDetail = ({
  documentId,
}: UseDocumentEngagementDetailProps): UseDocumentEngagementDetailReturn => {
  ...
  const { data, loading } = useDocumentEngagementQuery({
    variables: {
      documentId,
    },
  });
  ...
  const filteredAndSortedUsers = useMemo(() => {
    if (!data?.documentEngagement.users) return [];

    let filtered = data.documentEngagement.users;
  ...
  return {
    engagementData: data?.documentEngagement || null,
    filterStatus,
...
export default useDocumentEngagementDetail;
